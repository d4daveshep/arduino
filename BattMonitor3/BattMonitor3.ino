...
const int SAMPLE_DELAY = 100; // millisec delay between samples
...
int sampleCount = 0; // number of samples since startup
float secsElapsed = 0.0; // number of secs since startup
float totalAmps = 0.0; // total amps over all samples
float averageAmps = 0.0; // average amps over all samples
float ampHours = 0.0; // this is the total amp hours flowed since startup
...
//  amps = (((float)ampAverage * 5000.0 / 1024.0) - 500.0 ) / 133.0; // * 1000.0;
  amps = (((float)ampAverage * Vcc * 1000.0 / 1023.0) - 480.0 ) / 133.0;
  if(amps<0) {
    amps=0.0;
  }
  ...
  sampleCount++; // number of samples since startup
  Serial.print("#");
  Serial.print(sampleCount);
  Serial.print(" ");
  
  secsElapsed = millis() / 1000.0; // secs elapsed since startup - will overflow to zero in ~50 days
  Serial.print(secsElapsed);
  Serial.print("s ");
  
  totalAmps += amps; // total amps flowed since startup
  Serial.print(totalAmps);
  Serial.print("A ");
  
  averageAmps = totalAmps / sampleCount; // average amps over all samples since startup
  Serial.print(averageAmps);
  Serial.print("A ");
  
  ampHours = averageAmps * secsElapsed / 3600.0; // amp hours flowed
