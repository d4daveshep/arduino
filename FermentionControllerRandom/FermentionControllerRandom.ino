 ...
 *  Randomise the temp readings and only print every 
 */
...
void setup(void) {
  ...
  double firstReading = randomTemp(); //sensors.getTempCByIndex(0); // read the temp into index location
...
//    targetTemp -= 1.0;  // decrease target temp by 1C
  ...
  tempReadings[tempIndex] = currentTemp; //sensors.getTempCByIndex(0); // read the temp into index location
/*
  Serial.print("target=");
 ...
 * { "temp":21.38, "min":19.45, "max":22.89, "action":"COOL", "target":20.00, "timestamp":123456789 }
 */
